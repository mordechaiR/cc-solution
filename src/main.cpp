...
private:
    ...
    //primary data structures for word frequencies and median values
    ...
    void medianFromHistogram (int &row, bool &rowIsEven);
    
    //determine word counts (overall and unique) from the current tweet.
    void wordCounter (int &numUniqueWords, string &tweet, int &row);
    
public:
    //default constructor
    WordsAndMediansProcessor () {};
    
    //default destructor
    ~WordsAndMediansProcessor () {};
    
    //find the word frequencies and median unique words per tweet from the input file of tweets.
    void findWordsAndMedians (string &inputFile);
    
    //output the data on the words and their frequencies.
    void doWordOutput (string &wordOutputFile);
    
    //output the data on the median number of unique words per line.
    void doMedianOutput (string &medianOutputFile);
};

void WordsAndMediansProcessor::medianFromHistogram (int &row, bool &rowIsEven) {
...
void WordsAndMediansProcessor::wordCounter (int &numUniqueWords, string &tweet, int &row) {
...
void WordsAndMediansProcessor::findWordsAndMedians (string &inputFile) {
    ...
    while (getline(input, tweet)) {
        wordCounter( numUniqueWords, tweet, row );
        if (numUniqueWords != 0 ) {
            histogram[numUniqueWords]++;
            medianFromHistogram( row, rowIsEven );
            row++;
        }
    }
    input.close();
...
int main ( int argc, char *argv[] ) {
    ...
    solution.findWordsAndMedians( inputFile );
    solution.doWordOutput( wordOutputFile );
    solution.doMedianOutput( medianOutputFile );
}
